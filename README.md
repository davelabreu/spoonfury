...
Spoonfury is an \[🚧🏗️\_under construction\_🏗️🚧\] recipe-first social platform designed to bring joy and efficiency back to cooking and meal planning. No fluff, no endless stories or ads —— just recipes, ingredients, and method.
...
4.  **Verification**: Backend tests use `pytest`. Frontend is verified via Vite dev server.
5.  **Deployment**: Uses Docker Compose for consistent environments across development and production.
